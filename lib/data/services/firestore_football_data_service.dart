...
class FirestoreFootballDataService implements FootballDataService {
  FirestoreFootballDataService({FirebaseFirestore? firestore}) : _firestore = firestore ?? FirebaseFirestore.instance;
  ...
  Future<List<Team>> getTeams({required String competitionId}) async {
    final snapshot = await _competitionRef(competitionId).collection('teams').get();
  ...
  Future<List<Matchday>> getMatchdays({required String competitionId}) async {
    final snapshot = await _competitionRef(competitionId).collection('matchdays').orderBy('number').get();
  ...
  Future<List<TeamStanding>> getStandings({required String competitionId}) async {
    final doc = await _firestore.collection('standings').doc(competitionId).get();
  ...
  DocumentReference<Map<String, dynamic>> _competitionRef(String competitionId) =>
      _firestore.collection('competitions').doc(competitionId);

  Competition _competitionFromDoc(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    final data = doc.data();
    ...
    return Competition(
      ...
      status: _enumFrom(CompetitionStatus.values, data['status'] as String?, CompetitionStatus.active),
  ...
  Matchday _matchdayFromDoc(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return Matchday(
      ...
      status: _enumFrom(MatchdayStatus.values, data['status'] as String?, MatchdayStatus.upcoming),
  ...
  Match _matchFromData(DocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return Match(
      ...
      status: _enumFrom(MatchStatus.values, data['status'] as String?, MatchStatus.scheduled),
      homeScore: data['homeScore'] as int?,
      awayScore: data['awayScore'] as int?,
      createdAt: (data['createdAt'] as Timestamp?)?.toDate() ?? now,
      updatedAt: (data['updatedAt'] as Timestamp?)?.toDate() ?? now,
      predictionLocked: data['predictionLocked'] as bool? ?? true,
      winner: data['winner'] == null ? null : _enumFrom(MatchWinner.values, data['winner'] as String, MatchWinner.draw),
      goalNoGoal: data['goalNoGoal'] as bool?,
      overUnder: overUnderRaw?.map((key, value) => MapEntry(key, value as bool)),
    );
  }
